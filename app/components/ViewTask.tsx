...
const columns: TableProps<TodoType>['columns'] = [
  {
    ...
    key: 'title',
    width: 200,
    render: (_, text) => <Link href={`/view-task/${text.id}`}>{text.title}</Link>,
    className: "font-bold"
  },
  {
    title: 'Description',
...
const App: React.FC = () => {
  ...
  return <Table className='table-fixed px-5 m-5'  pagination={{pageSize:23}} columns={columns} dataSource={data} />;
};
